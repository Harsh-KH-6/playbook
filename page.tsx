          ...
          {filteredPlaybooks.map((playbook) => {
            ...
            return (
              <Card key={playbook.id} className="cyber-border">
                ...
                <CardContent>
                  {playbook.steps.find((step) => step.priority === "Info") && (
                    <div className="mb-6 p-4 rounded-lg bg-muted/50 border border-border">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-medium text-foreground">{playbook.steps.find((step) => step.priority === "Info").title[selectedLocale]}</h4>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleSpeak(`${playbook.id}-info`, playbook.steps.find((step) => step.priority === "Info").actions[selectedLocale].join(". "))}
                      ...
                      <div className="space-y-2 text-sm text-muted-foreground">
                        {playbook.steps.find((step) => step.priority === "Info").actions[selectedLocale].map((action, actionIndex) => (
